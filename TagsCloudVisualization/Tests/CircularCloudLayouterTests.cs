...
namespace TagsCloudVisualization
{
    ...
    public class CircularCloudLayouterTests
    {
        ...
        public void TearDown()
        {
            ...
            {
                var picturePath =
                    $@"{TestContext.CurrentContext.TestDirectory}\{TestContext.CurrentContext.Test.Name}.jpg";
                var bitmap = Visualizer.GetCloudVisualization(_cloud);
                bitmap.Save(picturePath, ImageFormat.Jpeg);
                Console.WriteLine($"Tag cloud visualization saved to file {picturePath}");
            }
        }
    ...
    {
        for (var i = 0; i < 300; i++)
            _cloud.PutNextRectangle(new Size(i+10, i+10));
        _cloud.Rectangles.Count.Should().Be(10);
    }
}
}