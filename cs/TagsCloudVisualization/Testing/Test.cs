...
class RectanglesNetTest
{
    ...
    public void SetUp()
    {
        _layouter = new CircularCloudLayouter(new Point(BITMAP_WIDTH/2, BITMAP_HEIGHT/2));
    }
    ...
    public void Rectangles_Should_Not_Intersect(int count)
    {
        GetRandomRectangles(count,  new Size(1,1),new Size(10, 10))
            .SelectMany(
                (x, i) => _layouter.Rectangles.Skip(i + 1), 
    ...
    public void Shape_Test_By_Areas_Compare(int count, float expectedDensity = 0.3f)
    {
        ...
        var approximatingCircleRadius = rects
            ...
            .Max(p => DistanceBetweenPoints(_layouter.Center, p));
    ...
    private List<Rectangle> GetRectangles(int n, Size size)
    {
        ...
        for (int i = 0; i < n; i++)
            result.Add(_layouter.PutNextRectangle(Size.Empty));
        return result;
    }
    ...
    private List<Rectangle> GetRandomRectangles(int n, Size minSize, Size maxSize)
    {
        ...
        var result = new List<Rectangle>();
        for (int i = 0; i < n; i++)
            result.Add(_layouter.PutNextRectangle(
                new Size(
